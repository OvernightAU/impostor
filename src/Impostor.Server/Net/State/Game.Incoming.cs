...
namespace Impostor.Server.Net.State
{
    internal partial class Game
    {
        ...
        private async ValueTask HandleJoinGameNew(ClientPlayer sender, bool isNew)
        {
            ...
            if (Host != null)
            {
                if (sender.Client.GameVersion != Host.Client.GameVersion && Host.Client.GameVersion != GameVersion.GetVersion(2019, 8, 16))
                {
                    string versionString = GameVersion.Version2String(Host.Client.GameVersion);
                    await sender.Client.DisconnectAsync(DisconnectReason.Custom, "You're not in the same version as host version.\nHost version is: " + versionString);
                    return;
                }
            }
